...
several different implementations of *how* to do it.
Now, we don't need to expose the _how_ to the clients that consume that algorithm. Notice, by client I mean any code
...
as needed, and we're ready to go and focus on our business logic.
...
To do that, let's take an example use case, so what we build here makes some sense.
...
- The application needs to be deployed in different environments where the internal email provider can be different.
...
But we face a challenge here: one of the requirements says the application needs to run in different environments
...
Now, let's make it work with one provider: let's say https://www.mailersend.com/[Mailersend].
